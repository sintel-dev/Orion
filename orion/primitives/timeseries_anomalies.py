...
def find_anomalies(errors, index, z_range=(0, 10), window_size=None, window_step_size=None,
                   min_percent=0.1, anomaly_padding=50, lower_threshold=False):
...
#     window_step_size = window_step_size or window_size
    ...
    # dyu
    window_size = len(errors) // 3
    window_step_size = window_size // 10
...
def find_anomalies_set(errors, index, z_range=(0, 10), window_size=None, window_step_size=None,
                   min_percent=0.1, anomaly_padding=50, lower_threshold=False):
    """Find sequences of error values that are anomalous.
    ...
    window_size = window_size or len(errors_set[0])
    window_step_size = window_step_size or window_size

    # dyu
    window_size = len(errors[0]) // 3
    window_step_size = window_size // 10
    print('length: {}, window_size: {}, window_step_size: {}'.format(len(errors_set[0]), window_size, window_step_size))
    ...
    for errors in errors_set:
        ...
        anomalies_set.append(np.asarray(anomalies))
