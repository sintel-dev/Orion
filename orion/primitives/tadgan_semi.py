...
LOSS_NAMES = [
    ['cx_loss', 'cx_real', 'cx_fake', 'cx_gp', 'cx_sup'],
    ['cz_loss', 'cz_real', 'cz_fake', 'cz_gp', 'cz_sup'],
...
class TadGAN(Model):
    ...
    def __getstate__(self):
        networks = []
        networks_remove = ['critic_x', 'critic_z', 'encoder', 'generator', 'critic_x_base',
                           'critic_x_sup', 'critic_z_base', 'critic_z_sup']
        modules = ['train_function', 'critic_x_optimizer', 'critic_z_optimizer',
                   'encoder_generator_optimizer'] + networks_remove
    ...
    def _build_tadgan(self, **kwargs) -> None:
        ...
        critic_x_sup_layer = tf.keras.layers.Dense(units=3)
        self.critic_x_sup = Model(x, critic_x_sup_layer(self.critic_x_base(x)))

        # Critic z
        self.critic_z_base = self._build_model(
            hyperparameters, self.layers_critic_z, self.critic_z_input_shape, name='critic_z')
        z = Input(shape=self.critic_z_input_shape)
        critic_z_discriminator_layer = tf.keras.layers.Dense(units=1)
        self.critic_z = Model(z, critic_z_discriminator_layer(self.critic_z_base(z)))
        critic_z_sup_layer = tf.keras.layers.Dense(units=3)
        self.critic_z_sup = Model(z, critic_z_sup_layer(self.critic_z_base(z)))
    ...
    def call(self, data: tuple, training=None, mask=None) -> tuple:
        X, y, labels = data
        z_ = self.encoder(X)
        critic_z_sup = self.critic_z(z_)
        y_hat = self.generator(z_)
        critic = self.critic_x(y)
        critic_x_sup = self.critic_x_sup(y)

        return y_hat, critic, critic_z_sup, critic_x_sup

    @tf.function
    def train_step(self, data: tuple) -> dict:
        X_train, y_train, labels_train = data
        ...
        for i in range(self.iterations_critic):
            ...
            l = labels_train[i * minibatch_size: (i + 1) * minibatch_size]
            ...
            with tf.GradientTape() as tape:
                ...
                cx_sup = self.critic_x_sup(x_)
                cx_sup_loss = SparseCategoricalCrossentropy()(l, cx_sup)

                cx_loss = cx_real_loss + cx_fake_loss + 10 * cx_gp + cx_sup_loss

            trainable_variables = self.critic_x_base.trainable_variables + \
                                  self.critic_x.trainable_variables[-2:] + \
                                  self.critic_x_sup.trainable_variables[-2:]
            ...
            batch_cx_loss.append([cx_loss, cx_real_loss, cx_fake_loss, cx_gp, cx_sup_loss])
            ...
            with tf.GradientTape() as tape:
                ...
                cz_sup = self.critic_z_sup(z_)
                cz_sup_loss = SparseCategoricalCrossentropy()(l, cz_sup)

                cz_loss = cz_real_loss + cz_fake_loss + 10 * cz_gp + cz_sup_loss

            trainable_variables = self.critic_z_base.trainable_variables + \
                                  self.critic_z.trainable_variables[-2:] + \
                                  self.critic_z_sup.trainable_variables[-2:]
            ...
            batch_cz_loss.append([cz_loss, cz_real_loss, cz_fake_loss, cz_gp, cz_sup_loss])
    ...
    def test_step(self, data) -> dict:
        ...
        batch_loss = [
            [cx_loss, cx_real_loss, cx_fake_loss, cx_gp, cx_sup_loss],
            [cz_loss, cz_real_loss, cz_fake_loss, cz_gp, cz_sup_loss],
    ...
    def fit(self, X: ndarray, y: Optional[ndarray] = None, labels = None, **kwargs) -> None:
        ...
        if self.validation_ratio > 0:
            ...
            train_data = (X[:-valid_length].copy(), y[:-valid_length].copy(), labels[:-valid_length].copy())
            valid = (X[-valid_length:].copy(), y[-valid_length:].copy(), labels[-valid_length:].copy())
    ...
    def predict(self, X: ndarray, y: Optional[ndarray] = None, labels = None) -> tuple:
        ...
        y = X.copy() if y is None else y
        X, y = X.astype(np.float64), y.astype(np.float64)
        test_data = (X, y, labels)
        y_hat, critic, critic_z_sup, critic_x_sup = self(test_data)
        # critic, critic_z_sup, critic_x_sup = critic.numpy(), critic_z_sup.numpy(), critic_x_sup.numpy()
        # critic *= 1 / (((critic_z_sup + critic_x_sup) / 2)[:, -1]).reshape((-1, 1))

        return y_hat.numpy(), critic.numpy(), y, self.fit_history.history, critic_z_sup.numpy(), \
               critic_x_sup.numpy()
