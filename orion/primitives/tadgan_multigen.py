...
LOSS_NAMES = [
    ...
    ['eg_loss', 'eg_cx_fake', 'eg_cz_fake', 'eg_mse', 'eg_cx_c']
]
...
class TadGAN(Model):
    ...
    def _build_tadgan(self, **kwargs) -> None:
        ...
        for i in range(self.num_generators):
            ...
            dense = tf.keras.layers.Dense(units=self.shape[0] // 2)
            generator = Model(x, self.generator_base(dense(flatten(x))))
    ...
    def call(self, data: tuple, training=None, mask=None) -> tuple:
        X, y = data
        z_ = self.encoder(X)
        y_hat = tf.reduce_mean([generator(z_) for generator in self.generators], 0)
    ...
    def train_step(self, data: tuple) -> dict:
        ...
        for i in range(self.iterations_critic):
            ...
            u = u_train[i * minibatch_size: (i + 1) * minibatch_size]

            # Train critic x.
            with tf.GradientTape() as tape:
                tensors = {'x_': [], 'cx_fake': [], 'y': [], 'cx_real': []}
                for j in range(self.num_generators):
                    x_ = self.generators[j](z[u == j], training=True)
                    cx_real = self.critic_x(y[u == j], training=True)
                    cx_fake = self.critic_x(x_, training=True)

                    tensors['x_'].append(x_)
                    tensors['cx_fake'].append(cx_fake)
                    tensors['y'].append(y[u == j])
        ...
        with tf.GradientTape() as tape:
            ...
            for j in range(self.num_generators):
                x_ = self.generators[j](z[u == j], training=True)
                cx_fake = self.critic_x(x_, training=True)
                z_ = self.encoder(x[u == j], training=True)
                cz_fake = self.critic_z(z_, training=True)
                x_rec_ = self.generators[j](z_, training=True)
