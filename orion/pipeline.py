...
class OrionPipeline(object):
    ...
    def tune(self, X, y, iterations=10):
        if not self._tuner:
            LOGGER.info('Scoring the default pipeline')
            self._best_score = self._score_pipeline(self._pipeline, X, y)
        ...
        for i in range(iterations):
            ...
            score = self._score_pipeline(candidate, X, y)
            LOGGER.info('Pipeline %s score: %s', i + 1, score)

            self._tuner.add(params, score)

            if self._db:
                self._db.insert_pipeline(candidate, score)
